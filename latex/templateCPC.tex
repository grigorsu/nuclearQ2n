...
\author{V.V.Sargsyan$^{1,2}$, S.Yu.Grigoryev$^{1}$, G.G.Adamian$^{1}$, N.V.Antonenko$^1$
}
\affiliation{$^{1}$Joint Institute for Nuclear Research, 141980 Dubna, Russia\\
$^{2}$Institut f\"ur Theoretische Physik der
...
The code uses the Fortran subroutine to calculate the nucleus-nucleus potential.
\end{abstract}

\pacs{25.70.Ji, 24.10.Eq, 03.65.-w \\ Key words:
sub-barrier fusion; dissipative dynamics; $S$-factor}
...
\end{eqnarray}
\begin{eqnarray}
J_{RR}(t)=\frac{2\omega\lambda}{\pi\mu}\int\limits_{0}^{\infty} dw\frac{\gamma^2  w}{\gamma^2+w^2}\coth(\hbar w/(2T))
\sum_{i,j}\beta_i\beta_j(s_i+\gamma)(s_j+\gamma)\psi(s_i,s_j,w,t),
\label{sigma}
\end{eqnarray}
...
\left[\frac{\pi T}{s_1\gamma}+\frac{1}{s_1-\gamma}
