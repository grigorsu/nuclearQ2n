// printf "\n\n\n\n\n\n\n"; g++ set_params.cc -o set_params.exe  -i conf.INP.txt -o conf.INP.txt
// printf "\n\n\n\n\n\n\n"; g++ set_params.cc -o set_params.exe  -A1 40 -Z1 20 -A2 40 -Z2 18 
//g++ -static -static-libgcc -static-libstdc++ set_params.cc -o w_set_params.exe
//g++ -static -static-libgcc -static-libstdc++ set_params.cc -o set_params.exe
...
int main(int argc, char** argv) {
  ...
  if(conf_args.is_present("-A1")) conf.change("A1", conf_args.get("-A1"));
  if(conf_args.is_present("-Z1")) conf.change("Z1", conf_args.get("-Z1"));
  if(conf_args.is_present("-A2")) conf.change("A2", conf_args.get("-A2"));
  if(conf_args.is_present("-Z2")) conf.change("Z2", conf_args.get("-Z2"));
  ...
  if(  (  XX.Z * YY.Z  < 150) || ( 2500 < XX.Z * YY.Z) ) {
    ...
    conf.change( "m1;"          ,      gFile.format_to_str(XX.m          ,10,"right") , 0 ) ;
    conf.change( "m2"          ,       gFile.format_to_str(YY.m          ,10,"right") , 0 ) ;
    conf.change( "Element1"    ,       gFile.format_to_str(XX.Element    ,10,"right") , 0 ) ;
    conf.change( "Element2"    ,       gFile.format_to_str(YY.Element    ,10,"right") , 0 ) ;
    conf.change( "beta1"       ,       gFile.format_to_str(XX.b          ,10,"right") , 0 ) ;
    conf.change( "beta2"       ,       gFile.format_to_str(YY.b          ,10,"right") , 0 ) ;
    conf.change( "d1"          ,       gFile.format_to_str(XX.d          ,10,"right") , 0 ) ;
    conf.change( "d2"          ,       gFile.format_to_str(YY.d          ,10,"right") , 0 ) ;
    std::string reaction = gFile.format_to_str(XX.A,3,"right") + XX.Element + "+" + YY.Element + gFile.format_to_str(YY.A,3,"left");
    conf.change( "reaction" , reaction    , 0 ) ;
    if(new_X != NULL) {
      std::cout <<" Q2n channel found: Q: " << Q << " E[2+]: " << E2 << std::endl;
      new_X->print();
      new_Y->print();
      conf.change( "Q"            ,    gFile.format_to_str( Q            ,10,"right")    , 0 ) ;
      conf.change( "E2"           ,    gFile.format_to_str( E2           ,10,"right")    , 0 ) ;
      conf.change( "A1_new"       ,    gFile.format_to_str(new_X->A      ,10,"right")    , 0 ) ;
      conf.change( "A2_new"       ,    gFile.format_to_str(new_Y->A      ,10,"right")    , 0 ) ;
      conf.change( "m1_new"       ,    gFile.format_to_str(new_X->m      ,10,"right")    , 0 ) ;
      conf.change( "m2_new"       ,    gFile.format_to_str(new_Y->m      ,10,"right")    , 0 ) ;
      conf.change( "beta1_new"    ,    gFile.format_to_str(new_X->b      ,10,"right")    , 0 ) ;
      conf.change( "beta2_new"    ,    gFile.format_to_str(new_Y->b      ,10,"right")    , 0 ) ;
      conf.change( "d1_new"       ,    gFile.format_to_str(new_X->d      ,10,"right")    , 0 ) ;
      conf.change( "d2_new"       ,    gFile.format_to_str(new_Y->d      ,10,"right")    , 0 ) ;
      conf.change( "Element1_new" ,    gFile.format_to_str(new_X->Element,10,"right")    , 0 ) ;
      conf.change( "Element2_new" ,    gFile.format_to_str(new_Y->Element,10,"right")    , 0 ) ;
      ...
      conf.change( "reaction_new" , reaction_new    , 0 ) ;
    }    
  ...
  if(conf_args.is_present("-print")) conf.print();  
