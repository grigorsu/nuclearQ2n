...
int nuclear::ffgg_find_root_print_state (size_t iter, gsl_multiroot_fsolver * s) {
    // std::cout <<             int(iter) << "  " << gsl_vector_get (s->x, 0) << "  " << gsl_vector_get (s->f, 0) << std::endl;
...
std::vector <double> nuclear::ffgg_roots(MNParameters &p, std::string option/*=""*/ /*"print"*/) {
    ...
    // cout $ option[2] $$
    // cout $ option[1+option.find("print")] $$
...
double nuclear::PELSS(double Ein, MNParameters &p, MPotential &potential, double * _omega=NULL ) {
    ...
    // double Efcm = Ein;
    // double Eful = Efcm - UBar;
    double Eful = Ein - potential.UBar;
      
    // DEBUG(" xMin = " << potential.xMin<<" xBar = " << potential.xBar<<" Eomega = " << Eomega << " xomega = " << xomega)
    // DEBUG(" Eful = " << Eful<<" Ein/Efcm = " << Ein<<" mu = " << p.mu<<"\n UMin = " << potential.UMin<<" UBar = " << potential.UBar )

      if (Ein <= potential.UMin) {
          double xE = potential.xMin;
          // double xS = find_solution(Ein, xBar + 1, RR[size-1]);
    ...
    // DEBUG(" xE = " << xE <<" xS = " << xS <<" S = " << S <<" p.omega = " << p.omega)
    ...
    // DEBUG(" xE = " << xE <<" xS = " << xS <<" S = " << S <<" p.omega = " << p.omega)
          ...
          // double xS = potential::eval_root(potential, Eomega,potential.xBar + 0.3/*,"print"*/);
	        ...
	        double S = potential.u_integrate(Eomega, double(p.mu.real()), xE, xS);
	        p.omega = double(p.pi.real())*(potential.eval(potential.xBar)-Eomega)/S;
    // DEBUG(" xE = " << xE <<" xS = " << xS <<" S = " << S <<" p.omega = " << p.omega << " potential.eval(potential.xBar)" << potential.eval(potential.xBar))
        }
    } 
    double PP = 0;
...
void nuclear::print(MNParameters &p, std::string option /*"details"*/) {
    ...
    // std::cout << "      Jr:  " <<       Jr( p.tlim , p )   << std::endl  ;
    // std::cout << "sigma_rr:  " << sigma_rr( p.tlim , p )   << std::endl  ;
    // // std::cout << "      r1:  " <<       r1( p.tlim , p )   << std::endl  ;
    if(option=="details") {
      ffgg_roots(p, "print");
    } else {
      ffgg_roots(p);
